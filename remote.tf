...
resource "null_resource" "testWebserver1HTTPD" {
  depends_on = [oci_core_instance.testWebserver]
  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      user        = "opc"
      host        = data.oci_core_vnic.testWebserver_VNIC1.public_ip_address
    ...
    inline = ["echo '== 1. Installing HTTPD package with yum'",
      ...
      "sudo /bin/su -c \"echo 'Welcome to Testing Page! This is WEBSERVER...' > /var/www/html/index.html\"",
